...
TODO check if we use A for arrival ac only, not A meaning all ac (like in the paper)
'''
...
t = model.addVars([(P_list[i], nodes[k]) 
                   for i in range(len(A)) 
                   for k in range(len(nodes))], 
                   vtype=GRB.CONTINUOUS, lb=0, name="t")
...
# Constraint 7: 
model.addConstrs(
(Z[i, j, u] <= gp.quicksum(Gamma[i, r] 
                           for r in R if u in N[r])
    for i in A 
    for j in A 
    for u in U),
...
model.addConstrs(
(Z[i,j,u] + Z[j,i,u] >= 
    2*(gp.quicksum(Gamma[i,r] 
                   for r in R if u in N[r]) +
    gp.quicksum(Gamma[j,r] 
                for r in R if u in N[r])) - 3
    for i in A 
    for j in A if i != j 
    for u in U),
...
arrival_node = {i: routes[i][0] 
                for i in routes}       # first node

model.addConstrs(
    (t[j, routes[j][0]] >= ETD[j] 
    for j in A_arrival),
name="arrival_time_window")

# Constraint 16: UPDATE with dataframe
departure_node = {i: routes[i][-1] 
                  for i in routes}    # last node

model.addConstrs(
    (t[i, routes[i][-1]] >= PBT[i] 
    for i in A_departure),
...
model.addConstrs(
    (t[i,v] - t[j,v] - (t[j,v] - t[j, w]) * find_separation(i,j) / length_edge((w, v)) >=
            - (3 - ((Z[j,i,v]) + gp.quicksum(Gamma[j, r] for r in routes_with_edge((w,v)))
                   + gp.quicksum(Gamma[i, r] 
                                 for r in R if v in 
                    N[r]) # Here need N[r] wich is the nodes in sequence for route r
                                 )) * M
                    for i in A 
                    for j in A if i != j 
                    for (w, v) in E),
...
model.addConstrs((
    ...
    for i in A
    for j in A 
    for c_k in c), name = 'runway_crossing_departure' )
