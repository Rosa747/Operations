...
def build_E_possible(R, possible_routes):
    E = {}
    for i, routes_i in possible_routes.items():
        E[i] = {}
        for p in routes_i:
            E[i][p] = R[p]["edges"]
    return E
...
# E = build_E_possible(R, { "AC1": ["route_arr_1a", "route_arr_1b"],
#                           "AC2": ["route_arr_1a", "route_arr_1b"],})
# print(E["AC1"]["route_arr_1a"])
...
t = model.addVars([(P.loc[P["id"] == [i]], nodes[k]) for i in P.loc["id"] for u in range(len(nodes))], vtype=GRB.CONTINUOUS, lb=0, name="t")
...
# arrival_node = {i: R.loc[R["name"] == i, "route"].iloc[0][0] for i in R["name"]}       # first node
# model.addConstrs(
# (t[j, P.at[idx, "routes"].loc[0][0][0]] >= ETD[j] for j in A_arrival),
